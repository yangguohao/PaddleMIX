# Copyright 2023 Black Forest Labs, The HuggingFace Team. All rights reserved.
#
...
class FluxTransformerBlock(nn.Layer):
    ...
    def __init__(self, dim, num_attention_heads, attention_head_dim, qk_norm="rms_norm", eps=1e-6):
        ...
        if hasattr(F, "scaled_dot_product_attention"):
            ...
            raise ValueError(
                "The current Pypaddle version does not support the `scaled_dot_product_attention` function."
            )
        self.attn = Attention(
            query_dim=dim,
            cross_attention_dim=None,
        ...
        self.norm2 = paddle.nn.LayerNorm(normalized_shape=dim, epsilon=1e-06, weight_attr=False, bias_attr=False) # nn.LayerNorm(dim, elementwise_affine=False, epsilon=1e-6)
        self.ff = FeedForward(dim=dim, dim_out=dim, activation_fn="gelu-approximate")

        self.norm2_context = nn.LayerNorm(normalized_shape=dim, epsilon=1e-06, weight_attr=False, bias_attr=False)
...
class FluxTransformer2DModel(ModelMixin, ConfigMixin): #  PeftAdapterMixin, FromOriginalModelMixin
    ...
    def __init__(
        ...
        self.x_embedder = paddle.nn.Linear(self.config.in_channels, self.inner_dim)
    ...
    def forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                ckpt_kwargs: Dict[str, Any] = {"use_reentrant": False} if is_paddle_version(">=", "3.0.0") else {}
